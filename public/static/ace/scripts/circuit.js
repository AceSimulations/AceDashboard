// Handle all Ace Airlines client actions
// TODO: Refine script
    ...
    function populateDepartures(destination, ident) {
        ...
        destination.departures.forEach((departure) => {
            ...
            if (ident === departure.uuid) {
                ...
                $(route).css({ "border": "3px solid #E60F0F" })
                $('#edit-icon').css({ "background-color": "#E60F0F" })
            ...
            $(expand).css({ height: "50%", position: "absolute", top: "50%", right: "20px", transform: "translateY(-50%)" })
            $(expand).attr('src', 'ace/expand.png')
            // Create and center the airplane image in each route wrapper
            var airplane = document.createElement('img')
            $(airplane).attr('src', 'ace/airplane.png')
            $(airplane).css({ width: "35px", transform: "rotate(90deg)", float: 'left', margin: "7px", "margin-left": "20px" })
    ...
    function buildCalendar(exports) {
        ...
        for (let x = 0; x < monthLength[date.getMonth()]; x++) {
            ...
            if (exports && exports.find((day) => day.day === x) || exports && exports.find((day) => day.day === x) === 0) { 
                $(calendar_element).css({ border: "2px solid #E60F0F" }).click((e) => {
